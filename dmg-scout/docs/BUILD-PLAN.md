...
**Reran eligibility on the real ten local Opportunities and updated `line_id` in place** (no ReasonBlock evidence text references the line by name, so nothing there needed changing): the five `split_dx` opportunities (ids 13/14/16/17/20) moved from ClimateMaster to LG; the five `rooftop_packaged` ones (11/12/15/18/19) were already AAON, unaffected. Same fix applied to the ten real Opportunities on production.

**Tests:** `tests/test_equipment_eligibility.py` -- rewrote the split_dx/vrf tests to assert LG only (confirming ClimateMaster and the other three vrf_split lines are correctly excluded, not just that ClimaCool is), added a new `water_source_heat_pump`-returns-ClimateMaster-only test. Full suite: 2,286 passed, 0 failed, 2 deselected.
