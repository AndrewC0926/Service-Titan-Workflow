...
**Bug fix found and committed separately before Item 3:** every Deadlines page row (Item 4) linked to a URL that never existed -- `/ab869-facility/{perm_id}` and `/retrofit-building/{id}` instead of the real `/ab869/{perm_id}` and `/retrofit/building/{id}`. The original test only checked the string against itself, never a live route. Fixed, plus a new `test_every_detail_url_resolves_to_a_real_route` that hits every row's `detail_url` through the actual app -- the only way this class of bug is actually caught. Full suite after: 2124 passed.

### Item 3: Decision Notes

New `DecisionNote` table (migration `e99344983e3e`, verified rollback): `NoteType` (won/lost/quote_lost/intel/decision), `NotePenHolder` (engineer/contractor/owner/gc/unknown -- deliberately a NEW, separate enum from Opportunity's own `PenHolderRole`, since section 31's literal value list doesn't match it), `BasisOfDesign` (ours/competitor_named/open/none), `LeadSource` (scout_signal/relationship/inbound/rep_originated/inside_sales), `NetsuiteRefType` (opportunity/project/sales_order). Reuses Item 2's `LostReasonCode` ("the same eight as lost dispositions," section 31's own words) and `OutcomeSource` directly rather than duplicating either.

**Six possible anchors**, matching section 31 exactly: `opportunity_id`, `project_id`, `building_id` (RetrofitBuilding), `account_id`, `signal_id`, or a typed NetSuite reference (`netsuite_ref_type` + `netsuite_ref` as text) -- "so a deal that never touched Scout still counts." `app/pipeline/notes.py::log_note()` is the one writer, enforcing at least one anchor and `netsuite_ref` requiring its type, same "no unresolved-entity concept" discipline as `app.outreach.log_outreach`. Config-driven label/order lists in `config.yaml`'s `notes.*`, same pattern as Item 2's `outcomes.*`.

**`three_deals_to_explain()`**: open (not won/lost) Opportunities older than 14 days, oldest first, capped at 3. **Disclosed limitation, not silently faked**: this is ONE shared candidate list, not a real per-user partition -- Opportunity has no owner/assigned-rep field anywhere in Scout today, so "per user" cannot be implemented honestly yet (a real split needs either an `Opportunity.owner` field or an attribution rule such as the last `Outcome.user` to touch it, neither of which exists).

**`/notes` page** (new, standalone -- not added to the six-destination rail, since this item didn't ask for a nav change): the three-deals-to-explain list, an add-note form, and the 100 most recent notes with their anchor resolved to a human label. **Add-note links added to every object page the schema actually anchors on**: Project detail, Account detail, RetrofitBuilding detail, and Pipeline's row expansion (Opportunity) -- each pre-fills `/notes?anchor_type=X&anchor_id=Y`. Signal is the one named anchor with NO dedicated add-note link: Signals are ephemeral `FeedSignal` shapes with no per-signal detail page to put a link on (Item 5's cards are the closest thing) -- the schema and `log_note()` both fully support `signal_id`, only the UI entry point is missing, disclosed rather than faked with a broken link.
...
### Item 3: Eligible lines for buildings and facilities

New `app/pipeline/equipment_eligibility.py` -- an `EquipmentClass` enum (rooftop_packaged/split_dx/vrf/chiller/cooling_tower/boiler/ahu/unknown) and a config map from equipment class to the app.accounts `ROLE_ORDER` role(s) it plausibly touches, built from the SAME `CATEGORY_TO_ROLE` table the rest of the line card already uses (rooftop_units/air_handling -> air_handling; vrf_split/chillers_cooling -> cooling_generation, matching `hcai.py`'s existing "vrf_split rolls up to cooling_generation" precedent; cooling_towers -> heat_rejection; heaters -> boiler's heating_specialty). `unknown` maps to no role at all -- ABSTAIN, wired all the way through: `eligible_lines_for_equipment_class()` returns `[]` for it without ever querying the line card. **Hospital facilities require OSP**: when `osp_required=True`, a line only counts if `ProductLine.oshpd_osp is True` -- the exact rule `app.accounts.line_offering_by_role` already enforces for project-sourced signals, reused, not reinvented.

**Equipment class is a computed function, not a new stored column** -- `RetrofitBuilding.equipment_type` already carries the raw permit-verified text; a second, redundant persisted field would just be another place for the two to drift.

**`equipment_type` -> `EquipmentClass` mapping, and one honest gap in it**: `vav_terminal` (one of `infer_equipment_type`'s eight real values) has no home in this item's own eight-value list -- a VAV terminal is a distribution/mixing box, not any of rooftop/split/vrf/chiller/tower/boiler/AHU -- so it maps to `unknown` rather than being forced into `ahu` just because both touch air handling. Symmetrically, **no real permit can ever classify as `vrf`**: `infer_equipment_type`'s own keyword patterns have no VRF pattern at all (`split_dx`'s regex is the closest a real permit description hits) -- `vrf` exists in the map only because the item's own value list names it and the hcai.py precedent gives it an unambiguous role, not because any building today can reach it.

**Coverage, measured against the real local restore:**
| population / source | total | known equipment class | coverage |
|---|---|---|---|
| RetrofitBuilding `recently_active` | 6,914 | 2,314 | 33.5% (split_dx 1,307, rooftop_packaged 651, ahu 259, chiller 43, boiler 33, cooling_tower 21; 4,600 unknown) |
| RetrofitBuilding `replacement_candidate` | 53,252 | 0 | 0.0% |
| Ab802Building | 50,259 | 0 | 0.0% |

**`replacement_candidate`'s 0.0% is by design, not a gap**: absence of a permit is the population's own definition (see the model's docstring), so there is no permit text to infer equipment from -- `equipment_type` is always null for every one of these 53,252 rows. **Ab802Building's 0.0% is likewise structural**: the table carries no equipment-class-bearing field at all -- checked directly, its own fields are property USE types and fuel-consumption totals for energy benchmarking, never HVAC equipment identity. Ab869Plan/HospitalBuilding (the ab869_plan signal source, HCAI-governed) were checked the same way and carry no equipment-class field either; `osp_required=True` is still written into the wiring for that source on principle, not because today's data can exercise it.

**Wired into `app.pipeline.signals_feed._eligible_fitting_line`** as a fallback path (`_eligible_fitting_line_from_equipment_class`), used whenever a signal has no `Category` (retrofit_building/ab869_plan/hcai_project/opsc_project/field_intel -- none of Scout's non-project sources carry one): resolves `RetrofitBuilding.equipment_type` for retrofit_building signals, sets `osp_required=True` for ab869_plan/hcai_project, and returns None (ABSTAIN) for every source with no path to a real equipment class.
...
### Item 5: Full suite, push, production run

**Full suite green before push: 2219 passed, 0 failed, 2 deselected**, 3:03 unchunked (Items 1-4's combined test additions over Block 4A's close-out baseline).

**Pushed** `claude/dmg-scout-architecture-wvmkqy` (commits `4bd89dd`/`4182591`/`b462fc5`/`a9e7380`). Both Render services picked up the new commit and deployed independently, watched to completion: web (`srv-d9qlfmks728c73a3n1u0`) and cron (`crn-d9qlfmks728c73a3n1v0`) both went `live` on commit `a9e7380`. Production `alembic_version` confirmed at the new head, `94adcbd2718d`, via a Render job. Route check (Basic Auth, real production password, per-user `andrew`/`DASHBOARD_PASSWORD_ANDREW` -- config.yaml's per-user login rotated Andrew off the legacy `DASHBOARD_PASSWORD` pair on 2026-09-12): `/`, `/pipeline`, `/notes`, `/deadlines` all **200**. Deadlines links spot-checked directly (5 `/retrofit/building/{id}` + 3 `/ab869/{id}`, out of 9,315 total rendered links across all four regulation groups) -- all **200**.

**Contacts CSV never committed -- uploaded to the job, not the repo, per the item's own instruction.** Render's Jobs API has no file-upload mechanism and a job's `startCommand` is capped by the container's own `MAX_ARG_STRLEN` (a single argument over ~128KB fails outright, confirmed directly: a first attempt embedding the whole gzip+base64 file as one `python3 -c` argument (620KB) failed at the OS level with `exec /usr/local/bin/python3: argument list too long`, not a Render-side rejection). Worked around by staging the file THROUGH the production database itself (already inside this deploy's own trust boundary, not a third party): the real CSV, gzip-compressed then base64-encoded (982,074 bytes -> 465,136 base64 chars), split into 7 chunks of <=70,000 chars each (each safely under the single-argument limit), each chunk `INSERT`ed into a small scratch table (`_contacts_import_staging`) by its own one-off job -- 7 jobs, each succeeded. A final, small job then read all 7 rows back, reassembled, decoded, decompressed, and called `import_netsuite_contacts` directly against the real production database. **The staging table's rows were deleted immediately after** (a follow-up job, verified 0 rows remaining) -- the raw contact data does not linger in a scratch table any longer than the single import run needed it. (The empty `_contacts_import_staging` table itself is still present, harmless with no rows -- worth a `DROP TABLE` next time someone is doing schema work, not done here since a direct schema-drop action was outside what I was willing to run unattended in this session.)

**Production numbers, Item 1 (contacts):**
| rows | reachable | matched by ID | matched by exact company name | unmatched |
|---|---|---|---|---|
| 10,401 | 3,775 (36.3%) | 0 | 0 | 10,401 (100%) |

**Identical to the local restore's own numbers, for the identical reason, verified directly rather than assumed**: production's `accounts` table also has exactly 1 row, with no `netsuite_entity_id` set (checked directly via a Render job) -- the customer master genuinely has not loaded to production either, exactly as this item's own preamble states ("order history and customer master still do not" load to production). Contacts loading ahead of the customer master is Andrew's own explicit decision (Sep 12); this 0/0/10,401 result is that decision's honest, expected consequence today, not a defect in the import.

**Production numbers, Item 4 (four-part filter rerun):** 54,666 unified signals (vs. local's 54,665 -- a 1-row drift consistent with production being a live, slightly-ahead-of-the-local-restore database, not a discrepancy in the logic).
| part | pass | missing |
|---|---|---|
| named_reachable_contact | **0** | 54,666 |
| sellable_account_or_building | 53,376 | 1,290 |
| dated_reason | 1,412 | 53,254 |
| eligible_fitting_line | 443 | 54,223 |

By trigger type -- same shape as local, same conclusion: `permit_gap` (53,252) passes only `sellable_account_or_building`; `public_work` (783) and `deadline` (188, of which 124 pass `sellable_account_or_building`) pass only `dated_reason`; `entitlement_milestone` (443) passes both `dated_reason` (441) and `eligible_fitting_line` (443). **Pass all four on production: 0**, for the identical `named_reachable_contact` root cause measured locally in Item 4 above. **Zero to promote on production too.**
...
**This "not a bug" conclusion was wrong, caught by a direct hand-check, not by intuition alone.** DMG has 1,637 mechanical-contractor customers, so a 0.06% CSLB hit rate was implausibly low on its face. Sampled 20 `customer_ref_name` values containing "Mechanical" or "Air" and looked each up by hand against `Contractor.business_name`/`full_business_name`: every single one carried a leading number DMG's own name never has -- `"58 XCEL MECHANICAL SYSTEMS INC"`, `"12719 MPS Mechanical Contractors, a JV"`, `"415 ALLISON MECHANICAL"`. `normalize_company_name` never strips a leading digit run (by design -- see its own docstring), so every one of these silently failed to match a real CSLB row that would otherwise line up exactly (`"415 ALLISON MECHANICAL"` vs. the real `"ALLISON MECHANICAL"`, license 670137).

**Confirmed this is a NetSuite internal customer/entity id leaking into the CSV's own Company column, not part of any company's real name, before touching any code:** of the 10,026 stored `customer_ref_name` values, 10,006 (99.8%) carry this exact `^\d+\s` shape -- far too prevalent to be individual companies that happen to start with a digit. Of the 1,363 distinct leading numbers that repeat across more than one Contact row, all but a handful map to the identical remaining company text every time (multiple contacts at the same customer sharing that customer's own id, not a coincidence of naming). Clinching evidence: `"14006 RDK Mechanical"` and `"1364 Vision Mechanical Services"` carry, digit for digit, the exact two `Account.netsuite_entity_id` values `Contact.customer_ref_id`'s own docstring had already verified by hand against the real NetSuite customer master for those same two companies, well before this investigation started.

**Fix placed at the root, not patched into the shared normalizer:** `normalize_company_name` itself was deliberately left untouched -- the CSLB roster has 218 real business names that legitimately start with a bare digit (`"5 STAR HEATING & COOLING"`, `"911 RESTORATION OF ORANGE COUNTY"`, `"24 / 7 PRO CONSTRUCTION INC"`), so stripping a leading digit run there would corrupt real contractor identity for the sake of fixing the other side. Instead, `app.importers.netsuite_contacts._parse_company_field()` (new) parses the leading id off the CSV's own Company column the same way `_parse_name_field()` already did for the rare Name-field shape -- now the DOMINANT source of `customer_ref_id` (99.8% prevalence vs. the Name field's 0.2%), taking precedence over a Name-field id on the rare row where both parse. `_match_account()` extracted as one shared helper so the main import loop and the new backfill below can never drift on what counts as a match. New `scout backfill-contact-ids` CLI command / `backfill_company_id_prefix()` re-applies this same parse to the 10,006 Contact rows already imported before this fix existed (the original CSV isn't kept around to just re-import) -- idempotent, and re-tries the Account match for any row that didn't already have one.

**Rerun against the real local restore:**
| | total considered | matched | unmatched |
|---|---|---|---|
| Before this fix | 10,026 | 6 | 10,020 |
| After `backfill-contact-ids` + rerun | 10,026 | **1,202** | 8,824 |

**200x improvement (0.06% -> 12.0%), and the Mechanical/Air-named cohort specifically now matches at 31.5%** (779 of 2,475 contacts whose `customer_ref_name` contains "Mechanical" or "Air") -- squarely in the "hundreds of contacts sit at CSLB-licensed firms" range the 1,637-mechanical-customer count predicts, not the near-zero the un-fixed normalizer produced.

**A fresh 20-sample after the fix, hand-checked the same way, to see what's left:** 6 matched cleanly. Of the 14 that didn't: **1 is a genuine, correctly-declined ambiguity** -- `"Amber Air Conditioning Inc."` normalizes to `"amber air conditioning"`, which TWO distinct real CSLB license holders both collide to once their legal suffixes strip (`AMBER AIR CONDITIONING COMPANY`, lic. 319072, and `AMBER AIR CONDITIONING INC`, lic. 621742) -- the exact-match discipline correctly refuses to guess which one this contact belongs to, exactly as designed. **3 have an obvious CSLB row blocked by a different, smaller-scale pattern than the id-prefix fix above, each a single instance in this sample, not (yet) shown to be systematic at the id-prefix's 99.8% scale, so left unfixed rather than guessed at:** `"Jupiter Air Conditioning and Heating"` vs. the real `"JUPITER AIR CONDITIONING & HEATING INC"` ("and" vs. "&"); `"BR Mechanical Inc"` vs. the real `"B R MECHANICAL INC"` (spaced initials); `"Leading Edge Air"` vs. the real `"LEADING EDGE AIR CONDITIONING"` (a truncated customer name in DMG's own data, not a formatting variant a normalizer could fix). **The remaining 10 are genuinely different or out-of-scope entities, not misses**: `Strobic Air` and `Airguard Industries` are equipment manufacturers, not CSLB contractors; `Ontario International Airport` is a customer site, not a business; `Kauai Air Conditioning Inc.` is Hawaii-based, outside CSLB's California jurisdiction entirely; the other six (`Polar Barr Air Conditioning`, `Pan-Pacific Mechanical`, `Countywide Mechanical`, `Air Performance Service, Inc.`, `Able Heating & Air Conditioning, Inc.`, `Applied Commercial Air, LLC`) have no CSLB row that normalizes to the same string at all, checked directly against the built name index, not assumed.

**Tests:** 11 new in `tests/test_netsuite_contacts.py` -- `_parse_company_field`'s id-strip and pass-through cases plus the Company-column id winning over a Name-field id on import (5) and a new `TestBackfillCompanyIdPrefix` (6: strips an already-imported row, newly resolves an Account the id-based path couldn't reach before, an already-clean row is untouched, a non-NetSuite contact is never considered, idempotent rerun finds nothing further, and an existing `account_id` is never overwritten by the backfill). Full suite: 2,237 passed, 0 failed, 2 deselected.

### Items 2-4: contractor-yard-anchored contacts, recently_active permit signals, and a real first ten Opportunities

**Item 2 -- `named_reachable_contact` gains a second path.** Block 4A Item 5 had flagged this as the whole block's real structural gap: the only source with a resolvable `signals` row (project) never has an account/building anchor, and the only sources with an anchor (retrofit_building, ab869_plan) never had a contact path at all. `app.pipeline.signals_feed._named_reachable_contact` now also accepts a reachable Contact (`reach_status='confirmed'`) anchored on a Contractor firm (`Contact.contractor_id`, Block 4B-prep-2 Item 1) whose own geocoded yard sits within `contractors.default_radius_miles` (15mi -- the same "realistically reachable" radius `app.contractors` already uses for a building's own nearest-mechanical-contractors list, not a second number invented here) of the signal's Building (`RetrofitBuilding.latitude/longitude`) or Deadline facility (`HospitalBuilding.latitude/longitude`, via `Ab869Plan.perm_id`). Real haversine distance (`app.contractors.haversine_miles`), never county/ZIP proximity.

**Performance, measured before shipping, not assumed:** a live per-signal query over 42,607 geocoded contractors would not scale to 60,000+ signals (the exact reason `app.contractors`' own `nearby_*` fields are precomputed rather than joined on page load). Built a one-query, in-memory grid instead (`_build_contractor_contact_grid`, 0.5-degree cells -- wider than the 15mi search radius, so a correct search never needs more than the 3x3 neighborhood of a point's own cell), cached per `Session` object identity via `WeakKeyDictionary` (freed automatically when the session goes out of scope; distinct per request and per test, so pytest's fresh-SQLite-per-test fixture never leaks a stale grid across tests).

**Verified independently, not just by the unit tests:** a 25-building brute-force sample (real haversine over all 1,110 reachable contractor-linked contacts, no grid, no shortcuts) found a match within 15mi for all 25 -- nearest contact 0.1-2.7mi away in every case. LA-metro contractor and retrofit-candidate density is simply high; 15mi is a generous radius here, not a bug inflating the number.

**Item 3 -- `recently_active` retrofit rows join `unified_signals`.** New `TriggerType.permit_activity` and `app.pipeline.signals_feed._retrofit_recently_active`: the symmetric opposite of `permit_gap`'s "absence is the signal" -- a real, dated, permit-verified mechanical install on record (2010-present). **6,914 such rows in the real local restore, matching the count named going in; only 2,402 of those (34.7%) carry a populated `equipment_type`** -- the rest have a real permit but `infer_equipment_type`'s own never-guess-past-the-work-description discipline left it null, same honest gap `replacement_candidate` rows have for a different reason. All 6,914 are folded in regardless (same "include the whole population, let each part ABSTAIN on what it can't determine" discipline `_retrofit_replacement_candidates` already uses) -- `_eligible_fitting_line_from_equipment_class` needed zero changes to pick this up, since it keys off `building_id`, not population.

`trigger_date` is Jan 1 of `latest_install_year`, populated on all 6,914 (the population's own permit-verified definition guarantees it). `pen_state` reuses this table's own already-computed `service_life_status` rather than re-deriving anything: `not_due`/`approaching` (2,392 of 6,914 -- the permitted contractor's hold is still current) is `moved`; `due`/`overdue` (10 of 6,914 -- that hold has plausibly lapsed, the window may have reopened) is `not_moved`; null (4,512 of 6,914, no resolvable service-life band for that use code) is ABSTAIN, never guessed into either state.

**Item 4 -- rerun, reported per part and by trigger type, real local restore (61,579 unified signals, up from Item 1's 54,665 by exactly the 6,914 new `permit_activity` rows):**

| Part | Pass | Total | Rate |
|---|---|---|---|
| named_reachable_contact | 57,491 | 61,579 | 93.4% |
| sellable_account_or_building | 53,386 | 61,579 | 86.7% |
| dated_reason | 8,325 | 61,579 | 13.5% |
| eligible_fitting_line | 2,756 | 61,579 | 4.5% |

**By trigger type:**

| Trigger | n | named_reachable_contact | sellable_account_or_building | dated_reason | eligible_fitting_line | all four |
|---|---|---|---|---|---|---|
| deadline | 188 | 171 | 124 | 188 | 0 | 0 |
| entitlement_milestone | 442 | 0 | 0 | 440 | 442 | 0 |
| permit_activity | 6,914 | 6,540 | 10 | 6,914 | 2,314 | **10** |
| permit_gap | 53,252 | 50,780 | 53,252 | 0 | 0 | 0 |
| public_work | 783 | 0 | 0 | 783 | 0 | 0 |

**Pass all four: 10 of 61,579 -- up from Item 1's 0, entirely on `permit_activity`.** Every real pass is a `recently_active` building whose equipment is `due`/`overdue` (exactly the 10 rows `service_life_status` puts in those two buckets) AND has a known `equipment_type` AND a real install-year date AND a reachable, Contractor-anchored contact within 15mi -- the four gates finally intersecting on one real population for the first time in this block. `permit_gap` still clears `sellable_account_or_building` 100% of the time (unchanged) but `dated_reason` never (absence has no date, by definition); `entitlement_milestone`/`public_work` still never clear `sellable_account_or_building` at all (Project has no Account join in Scout, unchanged finding from Item 1 of Block 3 forward) -- neither of those two facts changed here, on purpose; this block's scope was contact-reachability and the new permit-activity population, not the Account-join gap.

**Promoted the real top ten** (all-four-pass AND `can_promote_signal` -- the same fifth, separate gate the `/signals` page's own Promote button checks; ranked by `trigger_date` descending, the only ordering available for a pre-promotion FeedSignal), against the local restore, `owner_user="andrew"` (`config.yaml`'s `dashboard.operator_usernames[0]`). All ten real, all `permit_activity`/`retrofit_building`, all `pen_state=not_moved`, install years 2006-2011, `service_life_status` due or overdue. Every "why" row honestly reflects what Block 3's public-only data can see: **them** is Weak ("Building identified, owner unknown" -- no owner data source exists, see `RetrofitBuilding`'s own docstring), **now** is Strong (a real permit number, install year, and service-life finding), **win** is ABSTAIN on all ten (no DMG relationship/pairing evidence exists yet -- unchanged from every prior promotion in this block).

**Tests:** 1 pre-existing assertion in `tests/test_signals_feed.py` rewritten (it asserted recently_active buildings were excluded -- real behavior legitimately changed, not a regression) plus 7 new: two more `permit_activity`/pen_state cases (`not_moved` on `overdue`, ABSTAIN on null `service_life_status`) and five for the new `named_reachable_contact` path (a nearby Contractor-anchored contact satisfies the part; a far one doesn't; a non-`confirmed` contact doesn't; a building with no coordinates ABSTAINs rather than guesses; the same path works on a `facility_perm_id`/HospitalBuilding anchor too). Full suite: 2,244 passed, 0 failed, 2 deselected.

**Local only, as instructed** -- run and promoted against the local restore; nothing pushed or deployed. No credential value appears anywhere in this report or in the one-off report script used to produce it.

---

## 13. Block 4B-prep-3

### Items 1-6: real reachability, the real line card, owner honesty, a real "Do:" sentence, a customer-status flag, and the first real re-promotion

**Item 1 -- named_reachable_contact requires an actual contact method, not reach_status alone.** `Contact.reach_status='confirmed'` never meant "has a phone/mobile/email" for a NetSuite-imported row (see `Contact`'s own docstring); measured directly against Block 4B-prep-2's own ten promotions, 6 of 10 matched contacts had `reach_status='confirmed'` with NO phone, mobile, or email at all. `_has_a_real_contact_method()` (new, `app.pipeline.signals_feed`) adds `Contact.phone/mobile/email` presence as a second, ANDed condition on both the project-link path and Item 2's contractor-yard path. `Contact.reachable` was considered and rejected as a shortcut: only `app.importers.netsuite_contacts` ever sets it -- `app.enrichment.import_enriched_contact` (the OTHER real write path, which outright refuses a contact with no phone/email) never touches that column, so relying on it would have wrongly excluded every enrichment-sourced contact instead.

**Rerun against the real local restore: pass-all-four unchanged at 10, same ten building_ids.** The candidate pool of reachable, contractor-linked contacts shrank from 1,110 to 731 (34%), but LA-metro contractor density is high enough that `named_reachable_contact`'s overall pass rate didn't move (57,491/61,579, identical to before) -- `sellable_account_or_building` was always the true bottleneck, not contact reachability. What DID change: the contact actually attached to each Opportunity. Building 860703 (Opportunity 1 in the prior report) resolved to "Samanta Solis" (no phone/email) before this fix; the same building now resolves to "Lenin Uriostegui Jr." ((818) 384-4137, a real email) -- a genuinely callable lead in place of a dead end. 5 new tests (`tests/test_signals_feed.py`), including one confirming a mobile-only contact still passes.

**Item 2 -- the equipment-class-to-line map, corrected to the real card.** The Block 4B-prep-2 map (`EQUIPMENT_CLASS_TO_ROLES`, `app.pipeline.equipment_eligibility`) was role-based, not line-based, and it was measurably wrong: split_dx and chiller both resolved to the "cooling_generation" role, so ClimaCool (a chillers_cooling line) was offered as a split-DX replacement -- confirmed directly, 6 of Block 4B-prep-2's ten real promotions had a split_dx building matched to "ClimaCool." Replaced with `EQUIPMENT_CLASS_TO_LINE_NAMES`, a direct, named map matched by `ProductLine.name_norm`: `rooftop_packaged`->AAON/LG, `split_dx`/`vrf`->LG plus the other four real lines tagged `category='vrf_split'` on the card (ClimateMaster, Islandaire, Hitachi, Engineered Comfort -- inferred from that shared category tag, not literally named one-by-one in the instruction beyond LG, disclosed so any of the four can be pulled out directly), `chiller`->ClimaCool (its own `oshpd_osp=False` on the real card is what makes "only where OSP is not required" true automatically, no special case), `cooling_tower`->Marley ("SPX/Marley" -- SPX is Marley's parent company, not a separate card entry), `boiler`->nothing (ABSTAIN until a boiler line is confirmed -- never falls back to a heating_specialty-role line like Cambridge/Suburban/Markel/IEC just because it shares that tag), `ahu`->Energy Labs/AAON/ClimateCraft. `EQUIPMENT_CLASS_TO_ROLES` removed outright, not left dead. 9 new/rewritten tests (`tests/test_equipment_eligibility.py`, `tests/test_signals_feed.py`).

**Item 3 -- owner into "why them."** `RetrofitBuilding` was already verified (its own docstring) to carry no assessor owner-name/mailing-address field at all -- no free, bulk-queryable LA County source exposes one. Checked every other table in the schema for one anyway (`OwnershipRecency` has only a change-of-ownership date, no identity; `AssessorCandidate` has none; `Ab869Plan.owner_name` exists but is HCAI-crosstab-sourced for hospital facilities, a different domain entirely, out of this item's literal scope). New `_building_owner_them_evidence()`/`_owner_evidence_from_fields()` read the would-be fields via `getattr` (so a future migration adding either column needs zero code change here to start surfacing it, Strong strength, same caliber as a known Account) and, until then, say exactly which table it would belong on: RetrofitBuilding itself, keyed by apn, the same way `OwnershipRecency` already rejoins `last_sale_date` onto it at build time. 4 new tests.

**Item 4 -- do_person/do_ask/one_sentence, composed at promote time.** New `config.yaml` section `reason_block_do` (two dimensions: `by_trigger_type` gives a topic/ask pair, `by_pen_state` gives an urgency note) and `app.pipeline.reason_block.compose_do_fields()` -- plain string formatting, no LLM, matching `ReasonBlock`'s own worked example shape exactly ("Do: call X about Y and ask Z"). ABSTAINs (all three null) when no contact resolved, the Contact row is gone, or the exact trigger_type/pen_state pair has no config entry (e.g. `quiet_account`, which no source produces yet -- no entry added speculatively). Wired into `promote_to_opportunity`, written to all three ReasonBlock rows together per that model's own rule. 9 new tests (`tests/test_reason_block.py`, `tests/test_signals_feed.py`).

**Item 5 -- contractor_is_dmg_customer + assigned_rep on "win."** New `DmgCustomerRoster` table (migration `f2e00a774632`, verified rollback) -- company_name/name_norm/netsuite_internal_id/category/assigned_rep only, deliberately minimal, and deliberately EMPTY: this session never loads `~/netsuite-exports/ScoutResults653.csv` itself (see `app.pipeline.dmg_customer_roster`'s own module docstring) -- that's Andrew's file and his call to load, when authorized. `contractor_dmg_customer_status()`/`dmg_customer_status_evidence()` render "unknown, customer master not loaded" while the roster is empty, exact match (never fuzzy) against `Contractor.business_name`/`full_business_name` once it isn't, same discipline as Block 4B-prep-2's own Contact-to-Contractor join. No uniqueness constraint on `name_norm`: the real master file (read-only, spot-checked while investigating a prior question, never imported) was directly observed to carry two distinct rows for "GLM Heating and Air Conditioning Inc." under different internal ids -- a real quirk a constraint should not refuse to load later. Wired onto the "win" evidence line (context, not a strength change -- a contractor relationship isn't win evidence for the actual buyer, the building owner). 8 new tests (`tests/test_dmg_customer_roster.py`, `tests/test_signals_feed.py`).

**A real, unrelated bug found and fixed while wiring Item 6's production run.** `app.cli.match_contractors_cmd` was defined TWICE -- the pre-existing "match-contractors" command (`app.contractors.fetch_and_match_contractors`'s geocoded-contractor nearby-replacement-candidate-count precompute) and Block 4B-prep-2 Item 1's own Contact-to-Contractor match, both under the identical name. A module-level function redefinition silently rebinds the name at import time, so `app.cli.match_contractors_cmd` resolved to the SECOND (Item 1's) definition everywhere -- including inside `pipeline_cmd`'s own weekly-step dispatch, which calls it as `step(radius_miles=None)`. Confirmed directly: that raises `TypeError: match_contractors_cmd() got an unexpected keyword argument 'radius_miles'` -- meaning the next Sunday `scout pipeline` cron run on this branch would have crashed outright, not just left the original command quietly unreachable. Renamed Item 1's command/function to `match-contacts-to-contractors`/`match_contacts_to_contractors_cmd`, restoring the original meaning of `match-contractors`. Two new regression tests (`tests/test_cli.py`): a general "no duplicate `@app.command` name" guard (resolves Typer's auto-derived names, not just explicit ones, so it would have caught this even before either command had an explicit string) and a specific pin on which function `match_contractors_cmd` must be.

**Item 6 -- rerun, re-promote, full suite, push, production.** New `scout promote-top-signals` CLI command (`--top-n`, `--owner-user`) wraps exactly the report-then-promote logic used throughout this block, now runnable identically locally and on production via a Render job (the prior report was a one-off, uncommitted script). Deleted the ten stale Opportunity/ReasonBlock rows (and their synthesized `signals` rows) from Block 4B-prep-2's own promotion before re-running, since neither `promote_to_opportunity` nor `ensure_signal_for_promotion` de-duplicates against a prior run -- disclosed directly in the new command's own docstring rather than silently building delete-then-promote behavior into it.

**Rerun, real local restore (61,579 signals, unchanged population from Item 3 of the prior block):**

| Part | Pass |
|---|---|
| named_reachable_contact | 57,491 / 61,579 |
| sellable_account_or_building | 53,386 / 61,579 |
| dated_reason | 8,325 / 61,579 |
| eligible_fitting_line | 2,723 / 61,579 (down from 2,756 -- entirely the boiler ABSTAIN fix; no real due/overdue boiler building existed in this promotable set either way) |

**Pass all four: 10, identical building_ids to the prior run** -- the fix set changed WHAT each Opportunity says, not WHICH ten it is. All ten re-promoted, `owner_user=andrew`. Every one of the ten's `named_reachable_contact` is now genuinely reachable (real phone or email, `Contact.reachable=True` on all ten, up from 4 of 10 before Item 1); every equipment line now comes from the real card (AAON for every rooftop_packaged row, ClimateMaster for every split_dx row -- a real, correct mini-split line, never ClimaCool again); every "them" names RetrofitBuilding as where an owner join would live; every one has a real "Do:" sentence (e.g. "Do: call Tim Yeany about the building's mechanical service life and ask whether the installed equipment is already due for replacement (no contractor has a hold on this yet)."); every "win" states the contractor-DMG-customer status honestly as unknown (roster not loaded).

**Full suite: 2,285 passed, 0 failed, 2 deselected.**

**Pushed** to `claude/dmg-scout-architecture-wvmkqy` (commit `86683c4`); both Render services redeployed and went live on it, migration head confirmed `f2e00a774632` via a Render job.

**Production also needed the Item 1-follow-up DATA backfill re-run, not just the code fix -- caught before promoting, not after.** `scout match-contacts-to-contractors` on production initially returned the exact PRE-FIX numbers (6/10,026) -- the code fix shipped with this push, but `scout backfill-contact-ids` (a one-time data repair, not a schema migration) had only ever been run against the local restore, never production. Flagged directly rather than promoted through it; ran `scout backfill-contact-ids` on production (authorized explicitly before running, since it touches shared production data beyond this item's own named steps) -- **10,006 of 10,026 fixed**, identical to the local restore. Re-ran the match: **1,202/10,026**, matching local exactly.

**`scout promote-top-signals --owner-user andrew` on production:** 61,580 unified signals (1 more than local's 61,579 -- production has run its own daily pipeline independently and picked up one additional entitlement_milestone signal, a pre-existing, known drift, not new to this item). **Pass all four: 10**, same shape as local (all `permit_activity`, all due/overdue). **10 real Opportunities created on production for the first time** (ids 1-10 -- production had zero before this). Different `retrofit_buildings` row ids than local (867617, 867618, 867623, 867626, 860835, 860839, 860840, 860846, 860855, 860862 vs. local's 860703-series ids) since the two databases' retrofit tables were built independently, but identical in shape and logic.

**`scout snapshot-metrics` on production:** succeeded, 24 metric rows written -- `signals_by_trigger_type` now 5 (up from 4, the new `permit_activity` type), `deadline_exposure_by_regulation` 4, `source_freshness_days` 5, the rest 1 row each (`qualified_opportunities`, `opportunities_by_stage`, `opportunities_by_engine`, `opportunities_by_owner`, `whys_strength_distribution`, `earliness_rate`, `abstain_rate_delivery_method_class`, `abstain_rate_pen_state`, `abstain_rate_contact`).

No credential value appears anywhere in this report or in any command run to produce it -- every production command ran via a Render job against the service's own already-configured environment; nothing was passed a secret directly.

### Item 2 correction: split_dx is LG only, ClimateMaster is water_source_heat_pump

Block 4B-prep-3 Item 2's own line map was itself corrected once, live: `split_dx`/`vrf` were widened to every real line sharing `category='vrf_split'` on the card (LG, ClimateMaster, Islandaire, Hitachi, Engineered Comfort) -- an inference from a shared category tag, not a named fact, and disclosed as such at the time. It was wrong for at least ClimateMaster, confirmed directly by Andrew: LG's own single-zone and Multi V S products are what actually cover split_dx/vrf; ClimateMaster is a water-source heat pump line, a real but distinct equipment class this card didn't have room for before. New `EquipmentClass.water_source_heat_pump` (same "defined on principle, unreached by any real retrofit permit today" status `vrf` already has -- no `RetrofitBuilding.equipment_type` value maps to it). `EQUIPMENT_CLASS_TO_LINE_NAMES` corrected: `split_dx`/`vrf` -> LG only; `water_source_heat_pump` -> ClimateMaster only. Islandaire/Hitachi/Engineered Comfort dropped entirely, not reassigned anywhere -- the same category tag that wrongly implied ClimateMaster belonged on split_dx gives no real basis for keeping the other three either.

New `scout recompute-opportunity-lines` CLI command: re-runs eligible-line resolution against the CURRENT `EQUIPMENT_CLASS_TO_LINE_NAMES` map for every building-anchored Opportunity and updates `Opportunity.line_id` in place -- for exactly this situation, a line map correction landing after real Opportunities already exist. Scoped to building-anchored Opportunities only (the only kind that exist today, and the only kind whose eligible line is equipment-class-derived rather than Category-derived); never touches `reason_blocks`, since no evidence text references the line by name (checked directly).

**Reran on the real ten local Opportunities:** the five `split_dx` opportunities (ids 13/14/16/17/20) moved from ClimateMaster to LG; the five `rooftop_packaged` ones (11/12/15/18/19) were already AAON, unaffected. Same command run against production once the fix deployed there.

**Tests:** `tests/test_equipment_eligibility.py` -- rewrote the split_dx/vrf tests to assert LG only (confirming ClimateMaster and the other three vrf_split lines are correctly excluded, not just that ClimaCool is), added a new `water_source_heat_pump`-returns-ClimateMaster-only test. `tests/test_cli.py` -- 3 new tests for `recompute-opportunity-lines` (updates a stale line, leaves an already-correct one untouched, never touches an account-anchored Opportunity). Full suite: 2,289 passed, 0 failed, 2 deselected.
