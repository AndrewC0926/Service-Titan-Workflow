...
def test_absorb_fills_delivery_method_from_signal(db_session, cfg):
    project = _proj()
    _absorb(db_session, cfg, project, _sig(delivery_method="design_bid_build"))
    assert project.delivery_method_llm_hint == "design_bid_build"


def test_absorb_never_overwrites_known_delivery_method_with_null(db_session, cfg):
    project = _proj(delivery_method_llm_hint="cm_at_risk")
    _absorb(db_session, cfg, project, _sig(delivery_method=None))  # a later doc that doesn't mention it
    assert project.delivery_method_llm_hint == "cm_at_risk"


def test_absorb_never_overwrites_known_delivery_method_with_a_different_one(db_session, cfg):
