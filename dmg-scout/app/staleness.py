...
Measured on the live board 2026-08-07: of 296 active projects, 83 have evidence
under six months old, 73 six to twelve, and 140 over twelve. Nearly half the board
is unverified, which is a fact about public filings rather than a fault — most of
these projects file once at entitlement and are silent for a year.
"""
...
from app.models import Project, ProjectSignal, Signal, utcnow
...
def stage_ages(session: Session, projects: list[Project],
    ...
    """Evidence age per project, in two queries regardless of board size."""
    now = now or utcnow()
    if not projects:
        return {}
    ...
    best: dict[int, tuple[datetime, bool]] = {}
    for link in links:
        s = signals.get(link.signal_id)
        if s is None:
            continue
        # Prefer the event date; fall back to when the signal was recorded, and
        # remember which so the row can say so rather than implying a precision
        # it does not have.
        when, from_event = (s.event_date, True) if s.event_date else (s.created_at, False)
        if when is None:
            continue
        current = best.get(link.project_id)
        if current is None or when > current[0]:
            best[link.project_id] = (when, from_event)

    out: dict[int, StageAge] = {}
    for p in projects:
        entry = best.get(p.id)
        if entry is None:
            # No linked signal with any date. Falls back to the project's own
            # last_signal_at rather than reporting unknown, but only that far —
            # created_at would date the row from when WE made it, which says
            # nothing about the filing.
            when = p.last_signal_at
            from_event = False
            if when is None:
                out[p.id] = StageAge(None, None, False)
                continue
        else:
            when, from_event = entry
