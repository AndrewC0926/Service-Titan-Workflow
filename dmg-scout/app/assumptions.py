...
def load_assumptions(cfg: Config, service_calls_coverage: dict | None = None,
    ...
    out.append(Assumption(
        ...
        value=(f"{cg.get('roles_with_gap')} of {cg.get('roles_checked')} building-systems roles have "
              f"zero lines with a confirmed current OSP. Detail on the two roles most likely to be "
              f"pitched: chillers: 0 of {cg_chillers_n} DMG lines hold a confirmed current OSP "
        ...
        source_detail=(
            ...
            "decide whether to call. Generalized 2026-08-19 from chillers/fans only (the first "
            "two roles researched) to all 13 -- the full per-role table is on /hospitals itself, "
            "not just this register. Chillers are DMG's category='chillers_cooling' lines "
